...
const ProductCard = ({
    ...
    const isContentFromOneWorldSync = get2xProfileLogoImage !== PRODUCT_DEFAULT_IMG && getUrlParams(referUrl)?.contentProvider?.includes('ONE_WORLD_SYNC');

    const aspectRatio = isContentFromOneWorldSync && {
        '--profileImg-aspect-ratio': '4/3'
    };

    return (
        <div {...n('container').withTestId().props}>
            <div {...n('imageWrapper', [size, isContentFromOneWorldSync ? 'customAspectRatio' : null]).withVariables(aspectRatio).props}>
                <Image
