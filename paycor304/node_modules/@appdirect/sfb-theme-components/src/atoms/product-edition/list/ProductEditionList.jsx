...
import { getProductEditionsByCartStatus } from '../../../tools/cart';
...
import { parsePricingAction, handleAddToCartAction, handleReplaceEdition, redirectToCart } from '../../../tools/callToActions';
import Grid from '../../grid/Grid';
import SlickSlider from '../../slick-slider/SlickSlider';
import ProductEditionCard from '../card/ProductEditionCard';
import UpgradeModal from '../../../components/upgrade-modal/UpgradeModal';
import DotsNav from '../../dots-nav/DotsNav';
import ReplaceEdition from '../../replace-edition/ReplaceEdition';
// IMPORTANT: (s)css should be imported after other atoms/components
import './styles/ProductEditionList.scss';

const n = createNamespace('ProductEditionList');
...
export const ProductEditionListComponent = ({
    ...
    const { userUuid, companyUuid } = user;
    const { cart, fetchCart } = cartPreviewStore();
    const [showReplaceEditionIndex, setShowReplaceEditionIndex] = useState(-1);
    const nonStackableProduct = !isAppStackable(applicationType);

    // hide all ReplaceEdition cards when an item was just added to cart
    useEffect(() => {
        const addToCartFunc = async () => {
            setShowReplaceEditionIndex(-1);
        };
    ...
    useEffect(() => {
        if (nonStackableProduct) {
            fetchCart(locale);
        }
    }, [fetchCart, nonStackableProduct, locale]);
    ...
    // for non-stackable, there should only be one edition in cart from same product
    const nonStackableEditionInCartItemId = nonStackableProduct && Object.values(editionsInCart)?.[0];
    const renderModal = () => {
        const handleClose = () => {
    ...
    const renderEdition = (item, index) => {
        ...
        const eppId = item?.plans?.[0]?.uuid;
        const isReplaceEditionInCart = !crossSellManagementEnabled && editionsReplace?.includes(eppId);
        const isAddedToCart = !!(nonStackableProduct && eppId && editionsInCart?.[eppId]);

        const replaceEditionInCart = () => {
            if (isReplaceEditionInCart) {
                handleReplaceEdition(
                    cart?.id,
                    nonStackableEditionInCartItemId,
                    eppId,
                    userUuid,
                    companyUuid,
                    nonStackableProduct,
        ...
        if (showReplaceEditionIndex === index) {
            return (
                <ReplaceEdition
                    onSelectEdition={replaceEditionInCart}
                    onViewCart={redirectToCart}
                    onExit={() => setShowReplaceEditionIndex(-1)}
                    editionName={item?.name}
                    ref={{ mainRef, titleRef }}
                    i18n={i18n.replaceEdition}
                />
            );
        }
        ...
        const onSelect = () => {
            if (isReplaceEditionInCart) {
                setShowReplaceEditionIndex(index);
            } else {
                onEditionSelect(item, configurator, setUpgradeDetails);
            }
        };
        ...
        <section {...n('wrapper').props} id={ANCHOR_PRODUCT_EDITIONS} ref={containerRef}>
            {(i18n.selectEditionTitle || i18n.viewAddons) && (
                <header {...n('header', headerModifiers).props}>
                    ...
                    {hasAddons && <a href={`#${ANCHOR_PRODUCT_ADDONS}`} {...n('headerLink').props}>{i18n.viewAddons}</a>}
                </header>
            )}
            <div {...n('content').props}>
                {carouselEnabled ? (
...
export const defaultEditionsProps = PropTypes.shape({
    ...
    i18n: PropTypes.shape({
        ...
        replaceEdition: PropTypes.object
    }),
