...
export function getCartApiEndpoint(locale) {
    ...
    const baseApiUrl = locale
        ? `${baseUrl}/api/checkout/v1/cart/preview?locale=${convertToJavaLocale(locale)}`
        : `${baseUrl}/api/checkout/v1/cart/preview`;
    return baseApiUrl;
}
...
function getListingUrlParams(locale, queryKey) {
    ...
    if (queryKey) {
        listingUrlParams.push(`${SEARCH_QUERY_KEY}=${queryKey}`);
    }
    if (locale) {
